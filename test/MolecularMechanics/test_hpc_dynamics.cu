...
int main(const int argc, const char* argv[]) {
  ...
  const std::vector<int> trpi_tiling(10000, 0);
  ...
  trpi_heat_bath.initializeRandomStates(PrecisionModel::DOUBLE, 9815734, 25, gpu);
  ...
  for (int i = 0; i < trpi_heat_bath.getAtomCount(); i+= 1024) {
    ...
    for (int j = 0; j < trpi_heat_bath.getRandomCacheDepth() * 3; j++) {
      gpu_result.push_back(trpi_heat_bath.getCachedRandomResult(PrecisionModel::DOUBLE, i, j,
                                                                HybridTargetLevel::DEVICE));
      cpu_result.push_back(xrs.gaussianRandomNumber());
    }
    gpu_gstate.push_back(trpi_heat_bath.getGeneratorState(i, HybridTargetLevel::DEVICE));
    cpu_gstate.push_back(xrs.revealState());
  }
  check(gpu_result, RelationalOperator::EQUAL, Approx(cpu_result).margin(tiny),
