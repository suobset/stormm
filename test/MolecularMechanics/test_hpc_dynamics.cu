...
int main(const int argc, const char* argv[]) {
  ...
  Thermostat lang_bath(ThermostatKind::LANGEVIN, 305.0);
  lang_bath.setAtomCount(105000);
  lang_bath.setRandomCacheDepth(5);
  lang_bath.initializeRandomStates(PrecisionModel::DOUBLE, 9815734, 25, gpu);
  std::vector<double> gpu_result, cpu_result;
  std::vector<ullint4> gpu_gstate, cpu_gstate;
  Xoshiro256ppGenerator xrs(9815734, 25);
  for (int i = 0; i < lang_bath.getAtomCount(); i+= 1024) {
    xrs.setState(lang_bath.getGeneratorState(i, HybridTargetLevel::HOST));
    for (int j = 0; j < 15; j++) {
      gpu_result.push_back(lang_bath.getCachedRandomResult(PrecisionModel::DOUBLE, i, j,
                                                           HybridTargetLevel::DEVICE));
      cpu_result.push_back(xrs.gaussianRandomNumber());
    }
    gpu_gstate.push_back(lang_bath.getGeneratorState(i, HybridTargetLevel::DEVICE));
