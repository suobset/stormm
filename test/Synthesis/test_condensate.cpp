...
//-------------------------------------------------------------------------------------------------
void checkATAFootprint(const Condensate &cdns, const SynthesisCacheMap &scmap,
...
int main(const int argc, const char* argv[]) {
  ...
  scmap.setCondensateWorkUnits(&cdns_dbl);
  section(1);
  ...
  check(zflt_dev, RelationalOperator::EQUAL,
        ...
        "in the Condensate differ from the original PhaseSpaceSynthesis, even when recorded in "
        "double-precision.", tsm.getTestingStatus());
  check(cdw_dbl.natr_insr_top, RelationalOperator::EQUAL, 27, "The number of all-to-reference "
  ...
  check(cdw_dbl.nata_insr_top, RelationalOperator::EQUAL, 27, "The number of all-to-all "
        "instructions for systems grouped by topology does not agree with the number of systems "
        "for a CPU-bound Condensate.", tsm.getTestingStatus());

  // Check that all-to-one instructions for vairous system groupings cover all systems.
  ...
  for (int i = 0; i < cdw_dbl.natr_insr_src; i++) {
    const int4 tinsr = cdw_dbl.atr_insr_src[i];
    const int tgroup = cdw_dbl.atr_group_src[i];
    for (int j = 0; j < tinsr.w; j++) {
      atr_occupancy_src[scmapr.csystem_proj[scmapr.csystem_bounds[tgroup] + tinsr.x + j]] = 1;
    }
  }
  for (int i = 0; i < cdw_dbl.natr_insr_top; i++) {
    const int4 tinsr = cdw_dbl.atr_insr_top[i];
    const int tgroup = cdw_dbl.atr_group_top[i];
    for (int j = 0; j < tinsr.w; j++) {
      atr_occupancy_top[poly_psw.common_ag_list[poly_psw.common_ag_bounds[tgroup] +
                                                tinsr.x + j]] = 1;
    }
  }
  for (int i = 0; i < cdw_dbl.natr_insr_lbl; i++) {
    const int4 tinsr = cdw_dbl.atr_insr_lbl[i];
    const int tgroup = cdw_dbl.atr_group_lbl[i];
    for (int j = 0; j < tinsr.w; j++) {
  ...
  // Check that all-to-all instructions for vairous sytem groupings match the coverage of a
  // direct listing.
  checkATAFootprint(cdns_dbl, scmap, SystemGrouping::SOURCE);
  checkATAFootprint(cdns_dbl, scmap, SystemGrouping::TOPOLOGY);
  checkATAFootprint(cdns_dbl, scmap, SystemGrouping::LABEL);
  ...
  Hybrid<double> atr_rmsd_a(rplan.getOutputSize(RMSDTask::REFERENCE));
  Hybrid<double> atr_rmsd_b(rplan.getOutputSize(RMSDTask::REFERENCE));
  Hybrid<double> atr_rmsd_c(rplan.getOutputSize(RMSDTask::REFERENCE));
  Hybrid<double> ata_rmsd_a(rplan.getOutputSize(RMSDTask::MATRIX));
  Hybrid<double> ata_rmsd_b(rplan.getOutputSize(RMSDTask::MATRIX));
  Hybrid<double> ata_rmsd_c(rplan.getOutputSize(RMSDTask::MATRIX));
  ...
  Hybrid<double> atr_rmsd_src_a(rplan.getOutputSize(RMSDTask::REFERENCE, SystemGrouping::SOURCE));
  Hybrid<double> atr_rmsd_src_b(rplan.getOutputSize(RMSDTask::REFERENCE, SystemGrouping::SOURCE));
  Hybrid<double> ata_rmsd_src_a(rplan.getOutputSize(RMSDTask::MATRIX, SystemGrouping::SOURCE));
  Hybrid<double> ata_rmsd_src_b(rplan.getOutputSize(RMSDTask::MATRIX, SystemGrouping::SOURCE));
  Hybrid<int> src_examples(sc.getSystemCount());
  std::vector<double> atr_rmsd_src_ans(rplan.getOutputSize(RMSDTask::REFERENCE,
                                                           SystemGrouping::SOURCE));
  int vresult_idx = 0;
  ...
  for (int i = 0 ; i < sc.getSystemCount(); i++) {
    ...
    vresult_idx = roundUp(vresult_idx, warp_size_int);
  }
  
  // Construct the all-to-all RMSD results
  std::vector<double> ata_rmsd_src_ans(rplan.getOutputSize(RMSDTask::MATRIX,
                                                           SystemGrouping::SOURCE));
  int isq_offset = 0;
  for (int i = 0 ; i < sc.getSystemCount(); i++) {
  ...
  rmsd(rplan, poly_ps, src_examples, &atr_rmsd_src_a, SystemGrouping::SOURCE);
  rmsd(rplan, poly_ps, &ata_rmsd_src_a, SystemGrouping::SOURCE);
  rmsd(rplan, poly_ps, cdns_dbl, src_examples, &atr_rmsd_src_b, SystemGrouping::SOURCE);
  rmsd(rplan, poly_ps, cdns_dbl, &ata_rmsd_src_b, SystemGrouping::SOURCE);
