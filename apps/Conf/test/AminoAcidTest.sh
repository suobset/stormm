...
&conformer
  ...
  trial_limit 1000, local_trial_limit 2000, final_states 6,
...
&minimize
  ncyc 50, cdcyc 0, maxcyc 1, ntpr = 1,
...
if [ -e ${STORMM_BUILD}/apps/Conf/conformer.stormm.cuda ] ; then
  ...
  valgrind ${STORMM_BUILD}/apps/Conf/conformer.stormm -O -i cgen.in -warn
fi
