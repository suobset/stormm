...
//-------------------------------------------------------------------------------------------------
template <typename T>
int findAlignmentWidth(const T* numbers, const size_t count, int decimal_places, size_t skip) {
  int result = 0;
...
//-------------------------------------------------------------------------------------------------
template <typename T>
int findAlignmentWidth(const std::vector<T> &numbers, int decimal_places, size_t skip) {
  return findAlignmentWidth<T>(numbers.data(), numbers.size(), decimal_places, skip);
}
