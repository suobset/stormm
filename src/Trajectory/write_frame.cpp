...
#if 0
      {
        std::string cryst_line("CRYST1");
        for (int i = 0; i < 3; i++) {
          cryst_line += realToString(box_dimensions[i], 9, 3, NumberFormat::STANDARD_REAL);
        }
        for (int i = 3; i < 6; i++) {
          cryst_line += realToString(box_dimensions[i], 8, 3, NumberFormat::STANDARD_REAL);
        }
        cryst_line += "\n";
        foutp->write(cryst_line.data(), cryst_line.size());
      }
#endif
      break;
    case CoordinateFileKind::AMBER_NETCDF:
...
//-------------------------------------------------------------------------------------------------
void writeFrame(std::ofstream *foutp, const std::string &filename, const Pdb &entry,
                const int model_index) {
  for (int i = 0; i < entry.getAtomCount(); i++) {
