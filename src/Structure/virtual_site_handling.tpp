...
void placeVirtualSites(Tcoord* xcrd, Tcoord* ycrd, Tcoord* zcrd, const double* umat,
  ...
  const Tcalc inv_gpos_factor = value_one / globalpos_scale_factor;
    ...
    case VirtualSiteKind::FLEX_2:
      {
        ...
        if (tcoor_is_sgnint) {
          cons Tcalc disp_mult = p_f2_factor * gpos_scale_factor;
...
void transmitVirtualSiteForces(const Tcoord* xcrd, const Tcoord* ycrd, const Tcoord* zcrd,
                               ...
                               const UnitCellType unit_cell, const VirtualSiteKit<Tcalc> &vsk,
  ...
  for (int i = 0; i < vsk.nsite; i++) {
    ...
    const Tcalc vs_frc[3] = { psw.xfrc[vsite_atom], psw.yfrc[vsite_atom], psw.zfrc[vsite_atom] };
    ...
    case VirtualSiteKind::FIXED_2:
      {
        ...
        imageCoordinates(&p_vs[0], &p_vs[1], &p_vs[2], umat, invu, unit_cell,
                         minimum_image);
        const Tcalc invr_p_f2 = 1.0 / sqrt((p_f2[0] * p_f2[0]) + (p_f2[1] * p_f2[1]) +
                                           (p_f2[2] * p_f2[2]));
    ...
    case VirtualSiteKind::FLEX_3:
      {
        ...
        xfrc[parent_atom] += (1.0 - p_f2_factor - p_f3_factor) * vs_frc[0];
        yfrc[parent_atom] += (1.0 - p_f2_factor - p_f3_factor) * vs_frc[1];
        zfrc[parent_atom] += (1.0 - p_f2_factor - p_f3_factor) * vs_frc[2];
        xfrc[frame2_atom] += p_f2_factor * vs_frc[0];
        yfrc[frame2_atom] += p_f2_factor * vs_frc[1];
        zfrc[frame2_atom] += p_f2_factor * vs_frc[2];
        xfrc[frame3_atom] += p_f3_factor * vs_frc[0];
        yfrc[frame3_atom] += p_f3_factor * vs_frc[1];
        zfrc[frame3_atom] += p_f3_factor * vs_frc[2];
      }
      break;
    case VirtualSiteKind::FIXED_3:
      {
    ...
    case VirtualSiteKind::FAD_3:
      {
        ...
        const Tcalc invr2_p_f2 = 1.0 / ((p_f2[0] * p_f2[0]) + (p_f2[1] * p_f2[1]) +
                                         (p_f2[2] * p_f2[2]));
        const Tcalc invr_p_f2 = sqrt(invr2_p_f2);
        const Tcalc invr2_t = 1.0 / ((f23_t_pf2[0] * f23_t_pf2[0]) +
                                     (f23_t_pf2[1] * f23_t_pf2[1]) +
                                     (f23_t_pf2[2] * f23_t_pf2[2]));
        const Tcalc invr_t = sqrt(invr2_t);
        const Tcalc f1fac  = dot(p_f2, vs_frc, 3) * invr2_p_f2;
        const Tcalc f2fac  = dot(f23_t_pf2, vs_frc, 3) * invr2_t;
        const Tcalc p_f2_factor = vsk.dim1[pidx] * cos(vsk.dim2[pidx]) * invr_p_f2;
        const Tcalc t_factor    = vsk.dim1[pidx] * sin(vsk.dim2[pidx]) * invr_t;
    ...
    case VirtualSiteKind::OUT_3:
      {
        ...
        const Tcalc mf2_01 = vsk.dim3[pidx] * (zcrd[frame3_atom] - zcrd[parent_atom]);
        const Tcalc mf2_02 = vsk.dim3[pidx] * (ycrd[frame3_atom] - ycrd[parent_atom]);
        const Tcalc mf2_12 = vsk.dim3[pidx] * (xcrd[frame3_atom] - xcrd[parent_atom]);
        ...
        const Tcalc mf3_01 = vsk.dim3[pidx] * (zcrd[frame2_atom] - zcrd[parent_atom]);
        const Tcalc mf3_02 = vsk.dim3[pidx] * (ycrd[frame2_atom] - ycrd[parent_atom]);
        const Tcalc mf3_12 = vsk.dim3[pidx] * (xcrd[frame2_atom] - xcrd[parent_atom]);
