...
VdwCombiningRule inferCombiningRule(const T* lja, const T* ljb, const int lj_type_count,
  ...
  for (int i = 0; i < lj_type_count; i++) {
    ...
    sig_six *= sig_six;
    epsilon[i] = 0.25 * ljb[i] / sig_six;
  }

  // Test for a Lorentz-Berthelot and geometric combining rules
  const std::vector<VdwCombiningRule> trials = { VdwCombiningRule::LORENTZ_BERTHELOT,
                                                 VdwCombiningRule::GEOMETRIC };
  for (size_t i = 0; i < trials.size(); i++) {
    bool test_passes = true;
    int ti = 1;
    while (test_passes && ti < lj_type_count) {
      int tj = 0;
      while (test_passes && tj < ti) {
        const double pred_eps = sqrt(epsilon[ti] * epsilon[tj]);
        double pred_sig;
        switch (trials[i]) {
        ...
        pred_sig_six *= pred_sig;
        const double pred_ljb = 4.0 * pred_eps * pred_sig * pred_sig;
        const double pred_lja = pred_ljb * pred_sig;
        const size_t tij_idx = (ti * lj_type_count) + tj;
        if (pred_ljb < constants::small) {
          test_passes = (test_passes && fabs(pred_lja - lja[tij_idx]) < constants::small &&
                         fabs(pred_ljb - ljb[tij_idx]) < constants::small);
        }
        else {
          test_passes = (test_passes &&
                         fabs(pred_lja - lja[tij_idx]) / lja[tij_idx] < constants::small &&
                         fabs(pred_ljb - ljb[tij_idx]) / ljb[tij_idx] < constants::small);
        }
        tj++;
      }
      ti++;
    }
    if (test_passes && ti == lj_type_count) {
      return trials[i];
    }
  }

  // Both known combining rules failed to describe the matrix.
...
VdwCombiningRule inferCombiningRule(const std::vector<T> &lja, const std::vector<T> &ljb,
  ...
  return inferCombiningRule(lja.data(), ljb.data(), n_lj_types, policy);
}
