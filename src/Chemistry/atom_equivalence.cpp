...
AtomEquivalence::AtomEquivalence(const AtomGraph &ag_in, const std::vector<double> &formal_charges,
                                 ...
                                 const std::vector<int> &chiral_centers, StopWatch *timer) :
    AtomEquivalence()
{
  ...
  ag_pointer = const_cast<AtomGraph*>(ag_in.getSelfPointer());

  // Loop over all atoms in each molecule of the topology, identifying common elements, then common
  // formal charges, free electron content, chirality, and ring inclusions, then common bonding
  // patterns.
  const ChemicalDetailsKit cdk = ag_in.getChemicalDetailsKit();
  std::vector<ChiralOrientation> chiralities(cdk.natom, ChiralOrientation::NONE);
  const size_t n_chir = chiral_centers.size();
  for (int i = 0; i < n_chir; i++) {
    const int atom_idx = abs(chiral_centers[i]) - 1;
    chiralities[atom_idx] = (chiral_centers[i] < 0) ? ChiralOrientation::RECTUS :
  ...
  for (int hmol = 0; hmol < cdk.nmol; hmol++) {
    ...
    findEquivalentAtoms(ag_in, hmol_atoms, &map_to_subset, formal_charges, free_electrons,
                        ring_inclusion, chiralities, &domain_coverage, &allowed_atoms,
                        &candidate_hopper, &domain_assignment, &jumbled_groups, &aligned_groups,
                        &layer_bounds, &a_idx_tree, &b_idx_tree, &a_zn_tree, &b_zn_tree,
                        &a_fc_tree, &b_fc_tree, &a_fe_tree, &b_fe_tree, &a_ri_tree, &b_ri_tree,
                        &a_ch_tree, &b_ch_tree, &a_coverage, &b_coverage);
  }
...
//-------------------------------------------------------------------------------------------------
void AtomEquivalence::findEquivalentAtoms(const AtomGraph &ag,
                                          ...
                                          std::vector<int> *a_idx_tree,
                                          std::vector<int> *b_idx_tree,
                                          std::vector<int> *a_zn_tree,
                                          std::vector<int> *b_zn_tree,
                                          std::vector<double> *a_fc_tree,
                                          std::vector<double> *b_fc_tree,
                                          std::vector<double> *a_fe_tree,
                                          std::vector<double> *b_fe_tree,
                                          std::vector<ullint> *a_ri_tree,
                                          std::vector<ullint> *b_ri_tree,
                                          std::vector<ChiralOrientation> *a_ch_tree,
                                          std::vector<ChiralOrientation> *b_ch_tree,
                                          std::vector<int> *a_coverage,
  ...
  for (int i = 0; i < nsubset_atom; i++) {
    ...
    std::vector<int> equivalence_partners;
    for (int j = i + 1; j < nsubset_atom; j++) {
      const int atom_j = subset_atoms[j];
      if (in_symmetry_group[j]) {
        continue;
      }
      if (zn_i == cdk.z_numbers[atom_j] && fabs(fc_i - formal_charges[atom_j]) < 1.0e-6 &&
          fabs(fe_i - free_electrons[atom_j]) < 1.0e-6 && ri_i == ring_inclusion[atom_j] &&
          ch_i == chiralities[atom_j] &&
          matchBondingPattern(ag, formal_charges, free_electrons, ring_inclusion, chiralities,
                              atom_i, atom_j, a_idx_tree, b_idx_tree, a_zn_tree, b_zn_tree,
                              a_fc_tree, b_fc_tree, a_fe_tree, b_fe_tree, a_ri_tree, b_ri_tree,
                              a_ch_tree, b_ch_tree, a_coverage, b_coverage)) {

        // These two atoms are equivalent.  Add them to a growing list of equivalent atom
        // candidates.
        if (equivalence_partners.size() == 0) {
          equivalence_partners.push_back(atom_i);
        }
        equivalence_partners.push_back(atom_j);
      }
    }
    if (equivalence_partners.size() > 0) {
      ...
      drawEquivalentGroups(equivalence_partners, subset_atoms, formal_charges, free_electrons,
                           ring_inclusion, chiralities, domain_coverage, allowed_atoms,
                           candidate_hopper, domain_assignment, jumbled_groups, aligned_groups,
                           layer_bounds, a_idx_tree, b_idx_tree, a_zn_tree, b_zn_tree, a_fc_tree,
                           b_fc_tree, a_fe_tree, b_fe_tree, a_ri_tree, b_ri_tree, a_ch_tree,
                           b_ch_tree, a_coverage, b_coverage);
      ...
      for (int j = last_group_count; j < group_count; j++) {
        ...
        for (int k = 0; k < group_orders[j]; k++) {
          ...
          findEquivalentAtoms(ag, inner_subset_atoms, map_to_subset, formal_charges,
                              free_electrons, ring_inclusion, chiralities, domain_coverage,
                              allowed_atoms, candidate_hopper, domain_assignment, jumbled_groups,
                              aligned_groups, layer_bounds, a_idx_tree, b_idx_tree, a_zn_tree,
          ...
          for (int dom_idx = 1; dom_idx < n_partners; dom_idx++) {
            ...
            if (ncorr == 1) {
              ...
              for (int m = 0; m < ncorr; m++) {
                ...
                if (matchBondingPattern(ag, formal_charges, free_electrons, ring_inclusion,
                                        chiralities, atomk, atomm, a_idx_tree, b_idx_tree,
