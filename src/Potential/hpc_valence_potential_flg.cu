...
queryValenceKernelRequirementsLG(const EvaluateForce eval_frc, const EvaluateEnergy eval_nrg,
  ...
  if (cfa != cudaSuccess) {
    ...
    // Report the error
    rtErr(error_message, "queryValenceKernelRequirementsLG");
  }
...
//-------------------------------------------------------------------------------------------------
extern void launchValenceLG(const SyValenceKit<float> &poly_vk,
