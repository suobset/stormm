...
public:
  ...
  int2 getDensityMappingKernelDims(QMapMethod approach, PrecisionModel calc_prec,
                                   PrecisionModel acc_prec, size_t cg_tmat, int order) const;
  /// \}
...
private:
  ...
  std::vector<int> rac_qmap_block_multiplier_dp;
  std::vector<int> rac_qmap_block_multiplier_sp;
  ...
  ///        Descriptions of input parameters follow from catalogGeneralQMapKernel(), above.
  void catalogRegAccQMapKernel(PrecisionModel prec, size_t cg_tmat, int order,
                               const std::string &kernel_name);
  ...
  ///                   respectively
  void catalogShrAccQMapKernel(PrecisionModel calc_prec, PrecisionModel acc_prec, size_t cg_tmat,
                               int order, const std::string &kernel_name);
...
///        register-centered accumulation.  Each identifier begins with "qmap_racc_" and is then
///        appended with letter codes generated by appendQMapKernelKey(), above.  Descriptions of
///        input parameters also follow from that function.
std::string regAccQMapKernelKey(PrecisionModel prec, size_t cg_tmat, int order);
...
std::string shrAccQMapKernelKey(PrecisionModel calc_prec, PrecisionModel acc_prec, size_t cg_tmat,
                                int order);
