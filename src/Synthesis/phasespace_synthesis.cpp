...
PsSynthesisWriter::PsSynthesisWriter(const int system_count_in, const int unique_topology_count_in,
                                     ...
                                     const int* replica_idx_in, const float gpos_scale_in,
                                     const float vel_scale_in, const float frc_scale_in,
                                     const int gpos_bits_in, const int vel_bits_in,
    ...
    inv_gpos_scale{static_cast<float>(1.0 / gpos_scale_in)},
    inv_vel_scale{static_cast<float>(1.0 / vel_scale_in)},
    inv_frc_scale{static_cast<float>(1.0 / frc_scale_in)},
    gpos_bits{gpos_bits_in}, vel_bits{vel_bits_in}, frc_bits{frc_bits_in}, boxvecs{boxvecs_in},
    boxvec_ovrf{boxvec_ovrf_in}, umat{umat_in}, invu{invu_in}, boxdims{boxdims_in},
    alt_boxvecs{alt_boxvecs_in}, alt_boxvec_ovrf{alt_boxvec_ovrf_in}, umat_alt{umat_alt_in},
    invu_alt{invu_alt_in}, alt_boxdims{alt_boxdims_in}, xcrd{xcrd_in}, ycrd{ycrd_in},
    zcrd{zcrd_in}, xcrd_ovrf{xcrd_ovrf_in}, ycrd_ovrf{ycrd_ovrf_in}, zcrd_ovrf{zcrd_ovrf_in},
    ...
    yfrc_ovrf{yfrc_ovrf_in}, zfrc_ovrf{zfrc_ovrf_in}, xalt{xalt_in}, yalt{yalt_in}, zalt{zalt_in},
    xalt_ovrf{xalt_ovrf_in}, yalt_ovrf{yalt_ovrf_in}, zalt_ovrf{zalt_ovrf_in}, vxalt{vxalt_in},
    vyalt{vyalt_in}, vzalt{vzalt_in}, vxalt_ovrf{vxalt_ovrf_in}, vyalt_ovrf{vyalt_ovrf_in},
    vzalt_ovrf{vzalt_ovrf_in}, fxalt{fxalt_in}, fyalt{fyalt_in}, fzalt{fzalt_in},
    fxalt_ovrf{fxalt_ovrf_in}, fyalt_ovrf{fyalt_ovrf_in}, fzalt_ovrf{fzalt_ovrf_in}
{}
...
PsSynthesisReader::PsSynthesisReader(const int system_count_in, const int unique_topology_count_in,
    ...
    gpos_bits{gpos_bits_in}, vel_bits{vel_bits_in}, frc_bits{frc_bits_in}, boxvecs{boxvecs_in},
    boxvec_ovrf{boxvec_ovrf_in}, umat{umat_in}, invu{invu_in}, boxdims{boxdims_in},
    alt_boxvecs{boxvecs_in}, alt_boxvec_ovrf{alt_boxvec_ovrf_in}, umat_alt{umat_alt_in},
    invu_alt{invu_alt_in}, alt_boxdims{alt_boxdims_in}, xcrd{xcrd_in}, ycrd{ycrd_in},
    zcrd{zcrd_in}, xcrd_ovrf{xcrd_ovrf_in}, ycrd_ovrf{ycrd_ovrf_in}, zcrd_ovrf{zcrd_ovrf_in},
    xvel{xvel_in}, yvel{yvel_in}, zvel{zvel_in}, xvel_ovrf{xvel_ovrf_in}, yvel_ovrf{yvel_ovrf_in},
    zvel_ovrf{zvel_ovrf_in}, xfrc{xfrc_in}, yfrc{yfrc_in}, zfrc{zfrc_in}, xfrc_ovrf{xfrc_ovrf_in},
...
PsSynthesisReader::PsSynthesisReader(const PsSynthesisWriter *psyw) :
    ...
    inv_gpos_scale{psyw->inv_gpos_scale},
    inv_vel_scale{psyw->inv_vel_scale},
    inv_frc_scale{psyw->inv_frc_scale},
...
const PsSynthesisReader PhaseSpaceSynthesis::data(const CoordinateCycle orientation,
  ...
  case CoordinateCycle::WHITE:
    return PsSynthesisReader(system_count, unique_topology_count, unit_cell,
                             ...
                             velocity_scale_bits, force_scale_bits, box_vectors.data(tier),
                             ...
                             alt_box_dimensions.data(tier), x_coordinates.data(tier),
  ...
  case CoordinateCycle::BLACK:
    return PsSynthesisReader(system_count, unique_topology_count, unit_cell,
                             ...
                             shared_topology_instance_index.data(tier), globalpos_scale,
                             velocity_scale, force_scale, globalpos_scale_bits,
                             velocity_scale_bits, force_scale_bits, alt_box_vectors.data(tier),
                             ...
                             box_dimensions.data(tier), x_alt_coordinates.data(tier),
...
PhaseSpaceSynthesis::deviceViewToHostData(const CoordinateCycle orientation) const {
  ...
  case CoordinateCycle::WHITE:
    return PsSynthesisReader(system_count, unique_topology_count, unit_cell, devc_atom_starts,
                             ...
                             devc_shared_topology_instance_index, globalpos_scale, velocity_scale,
                             force_scale, globalpos_scale_bits, velocity_scale_bits,
...
PhaseSpace PhaseSpaceSynthesis::exportSystem(const int index, const HybridTargetLevel tier) const {
  ...
  // The export feature will always deliver the result to the HOST-side of the PhaseSpace object.
...
//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpaceReader &psr, const int system_index,
                                       const CoordinateCycle orientation,
                                       const HybridTargetLevel tier) {
  CoordinateCycle focus = orientation;
  importSystem(psr.xcrd, psr.ycrd, psr.zcrd, psr.umat, psr.invu, psr.boxdim, system_index, focus,
               1.0, TrajectoryKind::POSITIONS, tier);
  importSystem(psr.xvel, psr.yvel, psr.zvel, nullptr, nullptr, nullptr, system_index, focus, 1.0,
               TrajectoryKind::VELOCITIES, tier);
  importSystem(psr.xfrc, psr.yfrc, psr.zfrc, nullptr, nullptr, nullptr, system_index, focus, 1.0,
  ...
  importSystem(psr.xalt, psr.yalt, psr.zalt, psr.umat_alt, psr.invu_alt, psr.boxdim_alt,
               system_index, focus, 1.0, TrajectoryKind::POSITIONS, tier);
  importSystem(psr.vxalt, psr.vyalt, psr.vzalt, nullptr, nullptr, nullptr, system_index, focus,
               1.0, TrajectoryKind::VELOCITIES, tier);
  importSystem(psr.fxalt, psr.fyalt, psr.fzalt, nullptr, nullptr, nullptr, system_index, focus,
               1.0, TrajectoryKind::FORCES, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpaceReader &psr, const int system_index,
                                       const HybridTargetLevel tier) {
  importSystem(psr, system_index, cycle_position, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpaceWriter &psw, const int system_index,
                                       const CoordinateCycle orientation,
                                       const HybridTargetLevel tier) {
  importSystem(PhaseSpaceReader(psw), system_index, orientation, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpaceWriter &psw, const int system_index,
                                       const HybridTargetLevel tier) {
  importSystem(PhaseSpaceReader(psw), system_index, cycle_position, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpace &ps, const int system_index,
                                       const CoordinateCycle orientation,
                                       const HybridTargetLevel tier) {
  importSystem(ps.data(tier), system_index, orientation, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const PhaseSpace &ps, const int system_index,
                                       const HybridTargetLevel tier) {
  importSystem(ps.data(tier), system_index, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const CoordinateFrameReader &cfr, const int system_index,
                                       const CoordinateCycle orientation,
                                       const TrajectoryKind kind, const HybridTargetLevel tier) {
  importSystem(cfr.xcrd, cfr.ycrd, cfr.zcrd, cfr.umat, cfr.invu, cfr.boxdim, system_index,
...
//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const CoordinateFrameWriter &cfw, const int system_index,
                                       const CoordinateCycle orientation,
                                       const TrajectoryKind kind, const HybridTargetLevel tier) {
  importSystem(cfw, system_index, orientation, kind, tier);
}

//-------------------------------------------------------------------------------------------------
void PhaseSpaceSynthesis::importSystem(const CoordinateFrameWriter &cfw, const int system_index,
                                       const TrajectoryKind kind, const HybridTargetLevel tier) {
  importSystem(cfw, system_index, cycle_position, kind, tier);
}
...
void PhaseSpaceSynthesis::importSystem(const CoordinateFrame &cf, const int system_index,
                                       const CoordinateCycle orientation,
                                       const TrajectoryKind kind, const HybridTargetLevel tier) {
  importSystem(cf.data(tier), system_index, orientation, kind, tier);
}
